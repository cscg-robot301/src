...
#ifndef _countof
#define _countf(_Array) (int)(sizeof(_Array) / sizeof(_Array[0]))
#endif
...
namespace cscg{
    namespace drivers{
        ...
        bool stop_motor(std_srvs::Empty::Request &rep, std_srvs::Empty::Response &res){
            if(!drv){
                return false;
            }
        ...
        bool start_motor(std_srvs::Empty::Request &rep, std_srvs::Empty::Response &res){
            if(!drv){
                return false;
            }
...
int main(int argc, char* argv[]){
    ...
    private_nh.param<int>("serial_baudrate", serial_baudrate, 115200);
    private_nh.param<std::string>("frame_id", frame_id, "base_link");
    private_nh.param<bool>("inverted", inverted, false);
    private_nh.param<bool>("angle_compensate", angle_compensate, false);
    private_nh.param<std::string>("scan_mode", scan_mode, std::string());
    ...
    /// 获得雷达信息
    if(!cscg::drivers::getRPLIDARDeviceInfo(drv)){
        return -1;
    }

    /// 检查雷达是否正常
    if(!cscg::drivers::checkRPLIDARHealth(drv)){
        RPlidarDriver::DisposeDriver(drv);
        return -1;
    }

    ros::ServiceServer stop_motor_service = nh.advertiseService("stop_motor", cscg::drivers::stop_motor);
    ros::ServiceServer start_motor_service = nh.advertiseService("start_motor", cscg::drivers::start_motor);
    ...
    while (ros::ok()){
        ...
        if(op_result == RESULT_OK){
            ...
            if(op_result == RESULT_OK){
                if(angle_compensate){
                    ...
                    for (;i<count; i++){
                        if(nodes[i].dist_mm_q2 != 0){
                            ...
                            float angle = cscg::drivers::getAngle(nodes[i]);
                    ...
                    cscg::drivers::publish_scan(&scan_pub, angle_compensate_nodes, angle_compensate_nodes_count,
                    ...
                    angle_min = DEG2RAD(cscg::drivers::getAngle(nodes[start_node]));
                    angle_max = DEG2RAD(cscg::drivers::getAngle(nodes[end_node]));

                    cscg::drivers::publish_scan(&scan_pub, &nodes[start_node], end_node-start_node +1,
                                 start_scan_time, scan_duration, inverted,
                                 angle_min, angle_max, max_distance,
                                 frame_id);
                }
                ...
                cscg::drivers::publish_scan(&scan_pub, nodes, count,
                             start_scan_time, scan_duration, inverted,
                             angle_min, angle_max, max_distance,
                             frame_id);
            }
